...
def get_question_relevancy_task(task_input: TaskInput) -> Task:
    context_info = f" Consider this additional context: {task_input.query.context}" if task_input.query.context else ""
    output_file = task_input.output_file if task_input.output_file else Path("out" / "question_relevancy.txt")
    return Task(
        agent=task_input.agent,
        description=f"Evaluate if the question '{task_input.query.query}' is relevant for research. Consider factors such as clarity, specificity, research potential, and whether it is answerable through research. Flag questions that are too vague, nonsensical, or impossible to research effectively.{context_info}",
...
def get_research_approach_creation_task(task_input: TaskInput) -> Task:
    context_info = (
        f" Take into account this additional context: {task_input.query.context}" if task_input.query.context else ""
    )
    output_file = task_input.output_file if task_input.output_file else Path("out" / "research_approach.txt")
    return Task(
        agent=task_input.agent,
        description=f"Create a comprehensive research approach for the question: '{task_input.query.query}'. Outline the key areas to investigate, potential sources of information, and methodologies to employ. Consider different angles and perspectives that might provide valuable insights.{context_info}",
...
def get_search_query_generation_task(task_input: TaskInput) -> Task:
    ...
    return Task(
        agent=task_input.agent,
        description=f"Generate a diverse set of search queries related to the research question: '{task_input.query.query}'. Create at least 5 distinct search queries that will help gather comprehensive information. Queries should target different aspects of the question and use varying keywords to maximize relevant results.{context_info}",
...
def get_rag_retrieval_results_task(task_input: TaskInput) -> Task:
    ...
    return Task(
        agent=task_input.agent,
        description=f"Using Retrieval Augmented Generation (RAG), retrieve relevant information from the knowledge base to answer the research question: '{task_input.query.query}'. Focus on finding high-quality, accurate information that directly addresses the question and provides context.{context_info}",
...
def get_web_search_results_task(task_input: TaskInput) -> Task:
    ...
    return Task(
        agent=task_input.agent,
        description=f"Conduct comprehensive web searches using the generated queries to find the most relevant and up-to-date information related to the research question: '{task_input.query.query.query}'. Focus on authoritative sources, recent publications, and diverse perspectives.{context_info}",
